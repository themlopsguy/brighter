// app/onboarding/disabilityStatus.tsx
...
export default function OnboardingDisabilityStatus() {
  ...
  // Animation refs for each button (3 options for disability status)
  ...
  const handleDisabilityStatusSelect = (value: string) => {
    updateUserProfile({ disability: value });
  };
  ...
  return (
    <View style={styles.content}>
      <View style={[
        ...
        {/* Scrollable DisabilityStatus Selection Buttons */}
        <ScrollView
          style={[
          ...
          {disabilityStatusOptions.map((option, index) => {
            ...
            return (
              <Animated.View key={option.value} style={[animatedStyle, styles.animatedWrapper]}>
                <TouchableOpacity
                  style={[
                    styles.disabilityStatusButton,
                    { 
                      paddingVertical: responsiveValues.buttonPaddingVertical,
                      paddingHorizontal: responsiveValues.buttonPaddingHorizontal
                    },
                    userProfile?.disability === option.value && styles.selectedButton
                  ]}
                  onPress={() => handleDisabilityStatusSelect(option.value)}
                  activeOpacity={0.7}
                >
                  <Text style={[
                    styles.buttonText,
                    { fontSize: responsiveValues.buttonFontSize },
                    userProfile?.disability === option.value && styles.selectedButtonText
                  ]}>
                    {option.label}
                  </Text>
                </TouchableOpacity>
